...
SHARED_ACCOUNT = {
    ...
    # Features model to PD 0.127 -> grade D under interpret.taxonomy.grade_for,
    # so the pinned card, the drill-down re-score, and the memo all agree: an
    # account onboarded healthy that is now on the early-warning watchlist
    # (utilisation up, one facility overdue) but not yet an RFA.
    "features": {
        "disbursed_amount": 6000000,
        "asset_cost": 5200000,
        "ltv": 0.82,
        "PERFORM_CNS.SCORE": 690,
        "PRI.NO.OF.ACCTS": 4,
        "PRI.ACTIVE.ACCTS": 3,
        "PRI.OVERDUE.ACCTS": 1,
